...
namespace wyControls
{
    /// <summary>
    /// This control inherits from the standard Button.
    /// It allows you to add menus to your buttons similar to Office 2003 and Office 2007 buttons.
    /// This type of control is frequently called by other names such as "menu button,"
    /// "split button," and "drop down button."
    /// </summary>
    public class SplitButton : System.Windows.Forms.Button
    {
        ...
        private int borderWidth = SystemInformation.Border3DSize.Width * 2;
        private bool skipNextOpen;
        private bool isSplitMenuVisible;
        private TextFormatFlags textFormatFlags = TextFormatFlags.Default;


        private readonly int SplitSectionWidth = 18;
        
        #endregion Declarations

        #region Custom properties: SplitMenu, SplitMenuStrip, ShowSplit

        [DefaultValue(null), CategoryAttribute("Behavior"),
         Description("The System.Windows.Forms.ContextMenu to pop-up when handling the SplitButton click event, or null if there is no associated System.Windows.Forms.ContextMenu. The default is null. Although ContextMenuStrip replaces and adds functionality to the ContextMenu control of previous versions, ContextMenu is retained for both backward compatibility and future use if you choose.")]
        public System.Windows.Forms.ContextMenu SplitMenu
        {
        ...
        [DefaultValue(null), CategoryAttribute("Behavior"),
         Description("The System.Windows.Forms.ContextMenuStrip to pop-up when handling the SplitButton click event, or null if there is no associated System.Windows.Forms.ContextMenuStrip. The default is null.")]
        public System.Windows.Forms.ContextMenuStrip SplitMenuStrip
        {
            get
            {
        ...
        [DefaultValue(true), Browsable(true), CategoryAttribute("Appearance"),
         Description("Determines whether to display this button as a SplitButton.")]
        ...
        #endregion Custom properties: SplitMenu, SplitMenuStrip, ShowSplit
        ...
        protected override void OnPaint(PaintEventArgs pevent)
        {
            ...
            int internalBorder = borderWidth;
            Rectangle focusRect =
            ...
            if (RightToLeft == RightToLeft.Yes)
            {
                ...
                if (drawSplitLine)
                {
                    ...
                    g.DrawLine(SystemPens.ButtonShadow, bounds.Left + SplitSectionWidth, borderWidth, bounds.Left + SplitSectionWidth, bounds.Bottom - borderWidth);
                    g.DrawLine(SystemPens.ButtonFace, bounds.Left + SplitSectionWidth + 1, borderWidth, bounds.Left + SplitSectionWidth + 1, bounds.Bottom - borderWidth);
                }
            }
            else
            {
                ...
                {
                    // draw two lines at the edge of the dropdown button
                    g.DrawLine(SystemPens.ButtonShadow, bounds.Right - SplitSectionWidth, borderWidth, bounds.Right - SplitSectionWidth, bounds.Bottom - borderWidth);
                    g.DrawLine(SystemPens.ButtonFace, bounds.Right - SplitSectionWidth - 1, borderWidth, bounds.Right - SplitSectionWidth - 1, bounds.Bottom - borderWidth);
                }
            }
        ...
        public override Size GetPreferredSize(Size proposedSize)
        {
            ...
            if (m_ShowSplit)
            {
                ...
                if (!string.IsNullOrEmpty(Text) && TextRenderer.MeasureText(Text, Font).Width + SplitSectionWidth > preferredSize.Width)
                    return preferredSize + new Size(SplitSectionWidth + borderWidth * 2, 0);
            }
